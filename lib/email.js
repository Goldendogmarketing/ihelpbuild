  ...
  'drip-1': {
    ...
    html: (vars) => `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#111;color:#f0ece4;padding:40px;border-radius:8px;">
        ...
        <p style="color:#f0ece4;line-height:1.7;font-size:15px;">This single prompt has helped our clients identify thousands in wasted time. But what if you had <strong>25 more</strong> like this, organized by business function?</p>
        <div style="text-align:center;margin:32px 0;">
          <a href="${vars.cheatSheetUrl || '#'}" style="display:inline-block;padding:14px 36px;background:#c8a97e;color:#08080a;text-decoration:none;border-radius:6px;font-weight:700;font-size:16px;">Get the 25-Prompt Cheat Sheet — $7 →</a>
        </div>
        <p style="color:#f0ece4;line-height:1.7;font-size:15px;">Talk soon,<br><strong style="color:#c8a97e;">The iHelpBuild Team</strong></p>
        <div style="margin-top:32px;padding-top:16px;border-top:1px solid #2a2a30;text-align:center;">
    ...
    html: (vars) => `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#111;color:#f0ece4;padding:40px;border-radius:8px;">
        ...
        <ul style="color:#f0ece4;line-height:2;font-size:15px;">
          ...
          <li><strong style="color:#c8a97e;">Free bonus:</strong> the full 25-Prompt Cheat Sheet included</li>
        </ul>
        <div style="text-align:center;margin:32px 0;">
          <a href="${vars.ebookUrl || '#'}" style="display:inline-block;padding:14px 36px;background:#c8a97e;color:#08080a;text-decoration:none;border-radius:6px;font-weight:700;font-size:16px;">Get the Blueprint + Cheat Sheet — $27 →</a>
        </div>
        <p style="color:#f0ece4;line-height:1.7;font-size:15px;">Talk soon,<br><strong style="color:#c8a97e;">The iHelpBuild Team</strong></p>
        <div style="margin-top:32px;padding-top:16px;border-top:1px solid #2a2a30;text-align:center;">
          <p style="color:#555;font-size:12px;margin:0;"><a href="${vars.unsubscribeUrl || '#'}" style="color:#8a8690;">Unsubscribe</a></p>
...
async function sendBlastEmail(to, subject, body, vars = {}) {
  const { EMAIL_FROM_NAME, SMTP_USER } = process.env;
  const transporter = getTransporter();

  if (!transporter) {
    console.log('Email not configured — skipping blast email.');
    return false;
  }
  ...
  let finalBody = body;
  if (vars.name) {
    finalSubject = finalSubject.replace(/\{\{name\}\}/g, vars.name);
    finalBody = finalBody.replace(/\{\{name\}\}/g, vars.name);
  }
...
module.exports = { sendNotification, sendTemplateEmail, sendBlastEmail };
